...
df = pd.DataFrame(np.concatenate((frameNumber_All, np.concatenate((numBout_All, np.concatenate((TailX_VideoReferential_All, TailY_VideoReferential_All), axis=1)), axis=1)), axis=1), columns=['frameNumber', 'boutNumber'] + ['TailPosX' + str(i) for i in range(1, 11)] + ['TailPosY' + str(i) for i in range(1, 11)])
