...
def createDistanceBetweenFramesExcelFile(ZZoutputFolder, paramList):
  ...
  for wellId in range(0, numberOfWells):
    ...
    if type(finalDataArray) == int:
      finalDataArray = np.concatenate((np.transpose(np.array([[i for i in range(0, len(wellData['instantaneousDistance']))]])), np.transpose(np.array([[i/fps for i in range(0, len(wellData['instantaneousDistance']))]])), np.transpose(np.array([wellData['instantaneousDistance']]))), axis=1)
    else:
