...
class FreelySwimmingExperiment(QWidget):
  def __init__(self, controller):
    ...
    optionsLayout.setRowStretch(0, 1)
    optionsLayout.setColumnStretch(0, 1)
    optionsLayout.setColumnStretch(1, 1)
    recommendedImage = _ClickableImageLabel(self, QPixmap(os.path.join(curDirPath, 'freelySwimming.png')), lambda: controller.wellOrganisation(controller, False, False, False, False, False, True))
    optionsLayout.addWidget(recommendedImage, 0, 0)
    freeZebra2RadioButton = QRadioButton("Recommended method", self)
    btnGroup.addButton(freeZebra2RadioButton)
    freeZebra2RadioButton.setChecked(True)
    optionsLayout.addWidget(freeZebra2RadioButton, 1, 0)
    recommendedLabel = util.apply_style(QLabel("For videos taken in well controlled laboratory settings (no change in background at all, no obstructions, etc...)"), font=QFont("Helvetica", 12))
    recommendedLabel.setWordWrap(True)
    optionsLayout.addWidget(recommendedLabel, 2, 0)
    deepLearningImage = _ClickableImageLabel(self, QPixmap(os.path.join(curDirPath, 'deepLearningCenterOfMass.jpg')), lambda: controller.wellOrganisation(controller, False, False, False, False, False, True))
    optionsLayout.addWidget(deepLearningImage, 0, 1)
    DLRadioButton = QRadioButton("Deep Learning based tracking", self)
    btnGroup.addButton(DLRadioButton)
    optionsLayout.addWidget(DLRadioButton, 1, 1)
    DLLabel = util.apply_style(QLabel("For videos with changing background or brightness, or presence of obstructions (such as an experimenter moving a pipette)"), font=QFont("Helvetica", 12))
    DLLabel.setWordWrap(True)
    optionsLayout.addWidget(DLLabel, 2, 1)
    ...
    btnGroup.addButton(freeZebraRadioButton)
    ...
    nextBtn.clicked.connect(lambda: QDesktopServices.openUrl(QUrl("https://zebrazoom.org/documentation/docs/DLtracking/tailTracking")) if DLRadioButton.isChecked() else util.addToHistory(controller.chooseGeneralExperiment)(controller, freeZebraRadioButton.isChecked(), 0, 0, 0, 0, freeZebra2RadioButton.isChecked()))
