...
class Tracking2(zebrazoom.code.tracking.BaseTrackingMethod):
  ...
  def __init__(self, videoPath, wellPositions, hyperparameters):
    ...
    self._trackingHeadingDataPerWell = [np.zeros((self._hyperparameters["nbAnimalsPerWell"], self._lastFrame-self._firstFrame+1, 1)) for _ in range(len(self._wellPositions))]
...
zebrazoom.code.tracking.register_tracking_method('multipleFishKeepIdentities.tracking', Tracking2)
