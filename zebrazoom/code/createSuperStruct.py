...
def createSuperStruct(dataPerWell, wellPositions, hyperparameters, pathToOriginalVideo=''):
  ...
  for numWell in range(nbWells):
    if numWell not in dataPerWell:
      wellPoissMouv.append([[]])
      continue
  ...
  if not hyperparameters['createValidationVideo'] or hyperparameters.get('savePathToOriginalVideoForValidationVideo', False):
